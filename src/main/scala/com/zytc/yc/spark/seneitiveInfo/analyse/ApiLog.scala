...
object ApiLog {
    def api(config: Config, streamData: InputDStream[ConsumerRecord[String, String]]): Unit = {
        //对批次数据进行过滤分析操作
        val json = streamData.map(_.value()) //取出所有批次数据json
            ...
            .filter(_.infotype.toLowerCase == "apilog")
            .map(x => {
            var dataClass = 0
            ...
            .foreachRDD(rdd => {
                rdd.foreachPartition(it => {
                    try {
                        ...
                        val ppst = conn.prepareStatement("insert into biz_log_apilog(" +
